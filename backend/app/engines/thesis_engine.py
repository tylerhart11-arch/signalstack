...
THEME_LIBRARY = [
    ...
    ThemeDefinition(
        name="Fed easing cycle drives rate-sensitive assets",
        theme_family="Rates, Policy, and Financial Conditions",
        default_time_horizon="Medium-term (3-12 months)",
        default_expression_style="Directional long / rate-sensitive",
        keywords={
            "fed": 1.8,
            "rate cut": 3.2,
            "rate cuts": 3.2,
        ...
        match_groups=(
            ("fed", "rate cut", "rate cuts", "easing cycle", "cutting rates", "pivot", "dovish", "fed funds"),
            ("duration", "lower rates", "easing", "long end"),
        ),
        transmission_channels=(
            ThemeSignal(name="Front-end rates fall", rationale="Rate cuts push 2Y yields lower, pulling down the risk-free rate across durations."),
            ThemeSignal(name="Financial conditions ease", rationale="Lower policy rates loosen credit conditions and compress risk premia."),
            ThemeSignal(name="Duration relief", rationale="Long-duration assets re-rate as the discount rate falls."),
        ),
        sectors=(
            SectorIdea(name="Rate-sensitive equities", stance="Overweight", rationale="Real estate, utilities, and growth names benefit most from a falling discount rate."),
            SectorIdea(name="Regional banks", stance="Neutral to Overweight", rationale="Net-interest-margin pressure eases as the curve steepens."),
            SectorIdea(name="Short-duration credit", stance="Underweight relative", rationale="Less upside than longer-duration bonds as the short end rallies harder."),
        ),
        etf_exposures=(
            ExposureIdea(name="iShares 20+ Year Treasury Bond ETF", symbol="TLT", stance="Long", rationale="Direct duration expression of the rate-cut thesis."),
            ExposureIdea(name="Utilities Select Sector SPDR", symbol="XLU", stance="Long", rationale="High-dividend, rate-sensitive equities re-rate with lower discount rates."),
            ExposureIdea(name="iShares iBoxx Investment Grade Corporate Bond ETF", symbol="LQD", stance="Long", rationale="IG credit spreads often tighten as policy eases."),
        ),
        representative_stocks=(
            ExposureIdea(name="American Tower", symbol="AMT", stance="Long", rationale="Long-duration REIT with high rate sensitivity."),
            ExposureIdea(name="NextEra Energy", symbol="NEE", stance="Long", rationale="Rate-sensitive utility that re-rates on falling yields."),
            ExposureIdea(name="Realty Income", symbol="O", stance="Long", rationale="High-yield REIT re-rates positively as the risk-free rate falls."),
        ),
        catalysts=(
            ThemeSignal(name="FOMC signals cuts", rationale="Forward guidance or dot plot moves lower confirm the pivot."),
            ThemeSignal(name="Inflation data softens further", rationale="Lower CPI prints clear the runway for earlier or deeper cuts."),
            ThemeSignal(name="Labor market softening", rationale="Rising unemployment gives the Fed cover to ease more aggressively."),
        ),
        confirming_signals=(
            ThemeSignal(name="2Y yield falls", rationale="The front end is the most direct readout on policy expectations."),
            ThemeSignal(name="Yield curve steepens", rationale="Bull steepening confirms markets pricing cuts ahead."),
            ThemeSignal(name="Rate-sensitive sectors outperform", rationale="REITs and utilities leading is a classic early easing signal."),
        ),
        invalidation_signals=(
            ThemeSignal(name="Inflation re-accelerates", rationale="Sticky or rising inflation forces the Fed to hold or hike."),
            ThemeSignal(name="Labor market stays too hot", rationale="Strong payrolls push back the timeline for cuts."),
            ThemeSignal(name="Long end sells off despite cuts", rationale="Fiscal or term-premium concerns can prevent duration from rallying."),
        ),
        related_themes=("Yield curve steepening", "Rate-sensitive equity rotation", "Duration rally"),
        notes=("Most powerful when the Fed is cutting into falling inflation, not cutting into stagflation.",),
    ),
    ThemeDefinition(
        name="Tariff-driven stagflation pressures margins and growth",
        theme_family="Trade Policy, Inflation, and Supply Chains",
        default_time_horizon="Medium-term (3-12 months)",
        default_expression_style="Barbell: inflation hedge and defensive quality",
        keywords={
            "tariff": 3.2,
            "tariffs": 3.2,
            "trade war": 2.8,
        ...
        match_groups=(
            ("tariff", "tariffs", "trade war", "trade policy", "protectionism"),
            ("stagflation", "supply chain", "import costs", "margin pressure", "reshoring"),
        ),
        transmission_channels=(
            ThemeSignal(name="Import cost pass-through", rationale="Higher tariffs raise input costs and consumer prices, feeding into CPI."),
            ThemeSignal(name="Margin compression", rationale="Companies with global supply chains absorb cost increases or pass them through at volume loss."),
            ThemeSignal(name="Growth-inflation tradeoff", rationale="Simultaneous cost push and demand drag creates the stagflationary mix the Fed finds hardest to navigate."),
        ),
        sectors=(
            SectorIdea(name="Domestic-facing industrials", stance="Selective overweight", rationale="Reshoring beneficiaries with little import exposure win relative to global supply chains."),
            SectorIdea(name="Consumer discretionary", stance="Underweight", rationale="Price-sensitive demand and import exposure compound margin pressure."),
            SectorIdea(name="Energy and materials", stance="Overweight", rationale="Hard-asset sectors provide inflation hedging and are less tariff-exposed."),
        ),
        etf_exposures=(
            ExposureIdea(name="iShares TIPS Bond ETF", symbol="TIP", stance="Long", rationale="Inflation-linked bonds protect against the price-level component."),
            ExposureIdea(name="Energy Select Sector SPDR", symbol="XLE", stance="Long", rationale="Energy is a natural stagflation hedge."),
            ExposureIdea(name="Consumer Staples Select Sector SPDR", symbol="XLP", stance="Long", rationale="Pricing power and demand inelasticity help defensives in stagflation."),
        ),
        representative_stocks=(
            ExposureIdea(name="Nucor", symbol="NUE", stance="Long", rationale="Domestic steel producer benefits from import protection."),
            ExposureIdea(name="Caterpillar", symbol="CAT", stance="Neutral", rationale="Mixed: infrastructure tailwind offset by global supply chain exposure."),
            ExposureIdea(name="Dollar General", symbol="DG", stance="Underweight", rationale="Value-sensitive consumer with tariff-exposed merchandise faces margin squeeze."),
        ),
        catalysts=(
            ThemeSignal(name="Tariff announcements or escalation", rationale="New or expanded tariffs directly update import cost expectations."),
            ThemeSignal(name="CPI surprise above consensus", rationale="Inflation refusing to fall confirms the cost-push pressure."),
            ThemeSignal(name="GDP or PMI weakens alongside higher CPI", rationale="The simultaneous slowdown-inflation combo validates the stagflationary read."),
        ),
        confirming_signals=(
            ThemeSignal(name="Core goods inflation stays elevated", rationale="Goods prices not falling as expected confirms tariff pass-through."),
            ThemeSignal(name="Manufacturing PMI slips below 50", rationale="Growth drag is real alongside the inflation pressure."),
            ThemeSignal(name="Margin guidance weakens across sectors", rationale="Corporate earnings calls confirming cost pressure validate the thesis."),
        ),
        invalidation_signals=(
            ThemeSignal(name="Trade deal or tariff rollback", rationale="Any reversal breaks the key driver of the thesis."),
            ThemeSignal(name="Dollar strengthens sharply", rationale="DXY appreciation can offset import cost increases in USD terms."),
            ThemeSignal(name="Inflation falls anyway", rationale="Demand destruction absorbing the cost push would make stagflation a false alarm."),
        ),
        related_themes=("Commodity reflation", "Supply-chain reshoring", "Defensive rotation"),
        notes=("Most potent when tariffs are broad-based and retaliatory escalation is in play.",),
    ),
    ThemeDefinition(
        name="Recession risk drives defensive rotation",
        theme_family="Late-Cycle Risk, Defensives, and Capital Preservation",
        default_time_horizon="Tactical to medium-term (1-9 months)",
        default_expression_style="Defensive / quality over cyclicals",
        keywords={
            "recession": 3.2,
            "recession risk": 3.4,
            "slowdown": 2.4,
            ...
            "hard landing": 3.0,
            "risk off": 2.4,
            "flight to quality": 2.6,
            "quality": 1.6,
        },
        match_groups=(
            ("recession", "recession risk", "hard landing", "contraction", "slowdown"),
            ("defensive", "capital preservation", "risk off", "flight to quality", "quality"),
        ),
        transmission_channels=(
            ThemeSignal(name="Earnings revision cycle turns negative", rationale="Recession expectations pull forward analyst downgrades, especially in cyclicals."),
            ThemeSignal(name="Credit stress broadens", rationale="Weaker growth increases default risk and spread widening feeds into equity multiples."),
            ThemeSignal(name="Consumer and business confidence falls", rationale="Spending cutbacks and capex deferral amplify the initial growth scare."),
        ),
        sectors=(
            SectorIdea(name="Consumer staples", stance="Overweight", rationale="Inelastic demand and pricing power tend to preserve margins in downturns."),
            SectorIdea(name="Healthcare", stance="Overweight", rationale="Non-discretionary demand and defensive cash flows outperform in slowdowns."),
            SectorIdea(name="Cyclical industrials", stance="Underweight", rationale="Revenue and earnings are most exposed to volume declines."),
        ),
        etf_exposures=(
            ExposureIdea(name="Consumer Staples Select Sector SPDR", symbol="XLP", stance="Long", rationale="Classic defensive rotation destination."),
            ExposureIdea(name="Health Care Select Sector SPDR", symbol="XLV", stance="Long", rationale="Non-discretionary demand and durable cash flows."),
            ExposureIdea(name="iShares 7-10 Year Treasury Bond ETF", symbol="IEF", stance="Long", rationale="Safe-haven duration benefits from a growth-driven flight to quality."),
        ),
        representative_stocks=(
            ExposureIdea(name="Procter & Gamble", symbol="PG", stance="Overweight", rationale="Staples giant with pricing power and global scale."),
            ExposureIdea(name="Johnson & Johnson", symbol="JNJ", stance="Overweight", rationale="Defensive healthcare with reliable dividend history."),
            ExposureIdea(name="Caterpillar", symbol="CAT", stance="Underweight", rationale="Classic late-cycle industrial most exposed to a capex freeze."),
        ),
        catalysts=(
            ThemeSignal(name="Yield curve inverts deeper or fails to steepen", rationale="Persistent inversion is the most reliable recession leading indicator."),
            ThemeSignal(name="Credit spreads widen materially", rationale="HY and IG spread moves often lead equity weakness."),
            ThemeSignal(name="Unemployment claims rise", rationale="Labor market softening is the confirming signal the market waits for."),
        ),
        confirming_signals=(
            ThemeSignal(name="Defensive sectors take leadership", rationale="Staples and healthcare beating SPX confirms the risk-off rotation."),
            ThemeSignal(name="Treasury yields fall on flight-to-quality", rationale="Safe-haven demand for bonds is consistent with recession pricing."),
            ThemeSignal(name="Small caps underperform large caps", rationale="Financing-sensitive names suffer first in a growth scare."),
        ),
        invalidation_signals=(
            ThemeSignal(name="GDP and earnings hold up", rationale="If growth data stays above trend, recession risk fades quickly."),
            ThemeSignal(name="Fed delivers aggressive insurance cuts", rationale="Policy response can short-circuit a recession before it starts."),
            ThemeSignal(name="Credit markets stabilize", rationale="Spread stability or tightening removes the key financial amplifier."),
        ),
        related_themes=("Credit stress leads equities", "Quality factor rotation", "Duration rally on growth fears"),
        notes=("Strongest when credit and labor data are both moving in the same direction as the thesis.",),
    ),
    ThemeDefinition(
